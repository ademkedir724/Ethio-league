...
export async function GET(
    ...
    try {
        ...
        const images = await prisma.coachImage.findMany({
            where: { coachId: id },
...
export async function POST(
    ...
    try {
        ...
        const count = await prisma.coachImage.count({ where: { coachId: id } });
        if (count >= MEDIA_LIMITS.coach) {
            return unprocessableEntity({ error: `Coach image limit of ${MEDIA_LIMITS.coach} reached` });
        }

        const image = await prisma.coachImage.create({
            data: {
                coachId: id,
                imageUrl: url,
